...
Bash
git clone https://github.com/YourUsername/AutoPodTranscript.git
...
### 修复
- **移除全局超时限制**：删除 `socket.setdefaulttimeout(None)` 全局调用，避免影响 OAuth 等其他网络连接；上传函数内部已针对性处理大文件长连接。
- **修复裸异常捕获**：将 `check_file_in_gdrive` 中的裸 `except:` 改为 `except Exception`，防止意外吞掉 `KeyboardInterrupt` 等系统信号。
- **修复 Google Drive token 过期问题**：新增 `_refresh_creds_if_needed()` 方法，在查询和上传前统一刷新已过期凭证，解决多线程场景下出现 401 错误的问题。
- **修复文件扩展名硬编码**：新增 `get_audio_ext()` 方法，从 `enclosure` 的 `type` 属性和 URL 中提取真实扩展名，支持 `.mp3 / .m4a / .ogg / .opus / .aac / .flac / .wav` 等格式，不再一律写死为 `.mp3`。
- **修复剧集排序逻辑**：移除依赖标题字符串猜测顺序的启发式逻辑，改用 RSS `<pubDate>` 字段解析时间戳后降序排列，确保任意播客源均能正确排序。